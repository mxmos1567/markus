...
export function ShelfDetailPage() {
  ...
  async function onSave(event: React.FormEvent) {
    ...
    if (!shelf) return
    const updated = await shelves.update({ ...shelf, name, description })
    setShelf(updated)
  }
  ...
  async function onReserveToggle(slot: ShelfSlot) {
    if (slot.status === 'occupied') return
    const next = slot.status === 'reserved' ? 'free' : 'reserved'
    const updated = await slotRepo.setStatus(slot.id, next)
  ...
  return (
    <div className="space-y-8">
      ...
      <form onSubmit={onSave} className="glass-panel grid gap-4 rounded-sm p-6 md:grid-cols-2">
        <div>
        ...
        <div>
          <label className="mb-1 block text-xs uppercase tracking-wide text-mutedgray">Description</label>
          <input
            value={description}
          ...
          {selected ? (
            <div className="space-y-3 text-sm">
              ...
              <p className="text-mutedgray">
                Status: <span className="text-warmwhite">{selected.status}</span>
              </p>
              {selected.memoryId && (
                <p className="text-mutedgray">
              ...
              <p className="break-all text-mutedgray">/slot/{shelf.slug}/{selected.code}</p>
              {selected.status !== 'occupied' && (
                <Button variant="ghost" onClick={() => onReserveToggle(selected)}>
                  {selected.status === 'reserved' ? 'Mark as Free' : 'Reserve Slot'}
                </Button>
              )}
            </div>
          ) : (
